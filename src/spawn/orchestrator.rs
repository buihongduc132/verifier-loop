...
/// pi must remain in the parent's process session; detached process groups stall
/// the backend. We still provide a kill-group fallback for orphaned descendants
/// that inherit the stdout pipe.
#[cfg(unix)]
...
fn kill_process_group(leader_pid: u32) {
    unsafe { libc::killpg(leader_pid as i32, libc::SIGKILL); }
}
#[cfg(not(unix))]
...
async fn gather(
    ...
    for (vid, mut child, vdir, write_handle, _goal_file_guard) in children.drain(..) {
        ...
        let stdout_pipe = child.stdout.take();
        let stderr_pipe = child.stderr.take();
        // Capture the leader pid before wait so inherited pipe descriptors held by
        // orphaned descendants can be closed after the backend exits.
        ...
        let run = tokio::select! {
            ...
            status = child.wait() => {
                // Kill orphaned descendants before draining; cap the drain wait so a
                // backend that retains a pipe cannot hang the verifier loop forever.
                if let Some(pid) = leader_pid {
                    kill_process_group(pid);
                }
                let (stdout_buf, stderr_buf) = match tokio::time::timeout(
